...
| 자산 관리             | `Partial` | `/asset` 개요(순자산·증가 분해·투자/현금/평가손익 KPI·자산 증가 분해), `/asset/monthly` 월별 현황(요약·리스크·구성별 분해), `/asset/detail` 보유/거래 상세 통합, portfolio·trend 구현. 상단 월 선택+자산 탭과 좌측 하위 메뉴 재배치, `/asset/transactions`는 상세 탭으로 호환 redirect. 외부 시세/환율 API 없음, 거래 수정 UI 없음 | [m006](./milestones/m006-asset-data-stabilization.md), [m009](./milestones/m009-ia-restructure-and-new-insights.md) |
