...
| 자산 월별 현황         | `Done`    | `/asset/monthly`에서 월별 스냅샷 기반 총자산, 전월 대비, 보유 목록, 리스크 분포 표시. 같은 월 컨텍스트에서 그룹별 월말 자산 입력 패널과 로컬 임시저장 제공           | 입력 패널의 모바일/다크 모드 수동 검증 필요                           |
...
- 진행 이력:
  ...
  - 2026-05-09: 입력 패널을 소유자별/자산유형별 검토 흐름으로 보완하고 localStorage 임시저장/불러오기/삭제 후 업로드 흐름 추가
