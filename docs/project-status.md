...
| 자산 관리             | `Partial` | `/asset` monthly(스냅샷 총자산·전월 대비·보유·리스크 + 월말 입력 패널)·portfolio·trend·transactions 구현. 월별 현황은 총자산 변화(`ΔN=ΔC+ΔV`)와 투자 평가액 하위 분해(`ΔV=ΔP+ΔG`), 현금화/보유 변화 추정 인사이트를 표시. 월말 입력 패널은 소유자▸기관 묶음 스텝으로 재편. 외부 시세/환율 API 없음, 거래 수정 UI 없음 | [m006](./milestones/m006-asset-data-stabilization.md), [m009](./milestones/m009-ia-restructure-and-new-insights.md) |
...
| M009. 정보구조(IA) 재편 & 신규 인사이트 화면 | `진행 중` | [m009-ia-restructure-and-new-insights.md](./milestones/m009-ia-restructure-and-new-insights.md) | 2026-06-08 FR-C5를 자산 변화 분해로 재정의하고 월별 현황에 변화 인사이트 1차 구현 |
