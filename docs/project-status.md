...
- [x] 기본 test script와 테스트 프레임워크 결정
- [x] transaction/category/asset service 단위 테스트 추가
- [x] API route validation/error response 테스트 추가
- [x] 단계형 거래 입력 hook 테스트 추가
- [x] 주요 화면에 대한 수동 검증 체크리스트 문서화

### M2. 수입/지출 실제 데이터 흐름 정리
...
| 일시             | 범위                     | 명령/방법      | 결과 | 비고                                                                       |
| ---------------- | ------------------------ | -------------- | ---- | -------------------------------------------------------------------------- |
| 2026-05-06 22:26 | M1 lint                  | `pnpm lint`    | Pass | 기존 `monthly-summary-view.tsx` unused import warning 1건                  |
| 2026-05-06 22:26 | M1 자동 테스트           | `pnpm test`    | Pass | Vitest 6 files, 34 tests                                                   |
| 2026-05-06 22:06 | 검증 이력 일시 형식 변경 | 정적 문서 확인 | Pass | 검증 이력에 분 단위 시간을 남기도록 형식 변경                              |
| 2026-05-06 22:05 | 마일스톤 우선순위 변경   | 정적 문서 확인 | Pass | 검증 체계 구축을 M1로 상향하고 기존 기능 마일스톤을 순연                   |
