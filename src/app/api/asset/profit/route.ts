...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await holdingValueSnapshotService.getMonthlyProfitData(year, month);

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('Asset Profit API Error:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch asset profit data' },
      { status: 500 }
    );
  }
}
